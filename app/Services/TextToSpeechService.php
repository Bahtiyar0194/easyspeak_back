...
class TextToSpeechService
{
    ...
    public function textToSpeechStream($service, $text, $voice_id, $model){
        ...
        switch ($service) {
            case 'openai':
                ...
                $response = $client->request('POST', $apiUrl . '/audio/speech', [
                    'headers' => [
                        'Authorization' => 'Bearer ' . $apiKey,
                        'Content-Type' => 'application/json',
                    ],
            ...
            case 'elevenlabs':
                ...
                $response = $client->request('POST', "{$apiUrl}/v1/text-to-speech/{$voice_id}/stream?output_format=mp3_44100_128", [
                    'headers' => [
                        'xi-api-key' => $apiKey,
                        'Content-Type' => 'application/json',
                    ],
                    'json' => [
                        'text' => $text,
                        'model_id' => $model, // например eleven_multilingual_v2
                        'voice_settings' => [
                            'stability' => 0.5,
                            'similarity_boost' => 0.75,
                        ],
