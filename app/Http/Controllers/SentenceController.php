...
class SentenceController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $rules = [
            'sentence' => 'required|string|between:1,500',
            'sentence_kk' => 'required|string|between:1,500',
            'sentence_ru' => 'required|string|between:1,500',
    ...
    public function update(Request $request)
    {
        ...
        if($request['upload_new_sentence_audio_file'] == 'generate'){
            $rules['generate_edit_sentence_audio_file'] = 'required|string';
        }
        elseif($request['upload_edit_sentence_audio_file'] == 'true'){
            $upload_config = UploadConfiguration::where('material_type_id', '=', 2)
            ->first();
        ...
        if(isset($edit_sentence)){
            ...
            if($request['upload_edit_sentence_audio_file'] == 'generate'){;
                $binary = base64_decode($request->input('generate_edit_sentence_audio_file'));
                $file_name = uniqid() . '.mp3';
